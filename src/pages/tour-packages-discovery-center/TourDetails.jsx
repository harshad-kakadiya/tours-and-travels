...
// Minimal mock removed: now fetch from API
...
const TourDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchTour = async () => {
      try {
        // API doesn't expose single-by-id; fetch list and pick by id
                    ...
                    return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-xl font-bold text-foreground">₹{Number(p?.discountedPrice || p?.price || 0).toLocaleString()}</span>
                            {(() => {
                              ...
                              return hasDiscount && original ? (
                                  <span className="text-sm text-muted-foreground line-through">₹{original.toLocaleString()}</span>
                              ) : null;
                            })()}
                          </div>
                        </button>
              ...
              <Section title="Select Date">
                <div className="flex flex-wrap gap-2">
                  {(Array.isArray(tour?.availableDates) ? tour?.availableDates : (tour?.availableDates ? [tour?.availableDates] : [])).map((dRaw) => {
                    ...
                    const d = isNaN(dateObj.getTime()) ? String(dRaw) : dateObj.toLocaleDateString(undefined, { month: 'short', day: '2-digit', year: undefined });
                    return (
                        <button
                            key={String(dRaw)}
              ...
              <div className="lg:sticky lg:top-24 space-y-4">
                <div className="border border-border rounded-2xl p-6 bg-card/90 backdrop-blur shadow-md">
                    ...
                    {(() => {
                      ...
                      return (
                        ...
                        <span className="text-2xl font-bold text-foreground">₹{discounted.toLocaleString()}</span>
                            {hasDiscount && (
                                <span className="text-base text-muted-foreground line-through">₹{Number(original || 0).toLocaleString()}</span>
                            )}
                      </span>
                      );
                    })()}
                  ...
                  <ul className="text-sm text-muted-foreground mb-4 space-y-1">
                    <li className="flex items-center justify-between"><span>Package</span><span className="text-foreground font-medium">{selectedPackage?.from || 'Select package'}</span></li>
